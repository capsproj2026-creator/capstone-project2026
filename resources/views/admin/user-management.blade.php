...
@section('content')
    ...
    @php
        $statCards = [
            ...
            ['label' => \App\Models\User::STAFF_DISPLAY_LABEL, 'value' => $staffCount, 'icon' => 'briefcase', 'text' => 'text-violet-600', 'iconBg' => 'bg-violet-50 text-violet-600'],
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-6">
            ...
            <select name="type" class="rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                @foreach (['All', 'Student', 'Staff', 'Guard'] as $type)
                    <option value="{{ $type }}" @selected($typeFilter === $type)>{{ $type === 'All' ? 'All roles' : ($type === 'Staff' ? \App\Models\User::STAFF_DISPLAY_LABEL : $type) }}</option>
                @endforeach
            </select>
