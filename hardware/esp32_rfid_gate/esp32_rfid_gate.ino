/**
 * Capstone Smart Campus VMS — ESP32 + RC522 RFID Gate Client
 *
 * Wiring (adjust pins if needed):
 *   RC522  → ESP32
 *   SDA    → GPIO 5
 *   SCK    → GPIO 18
 *   MOSI   → GPIO 23
 ...
 *   RED_LED   → GPIO 26
 *   BUZZER    → GPIO 27
 *   GATE_RELAY→ GPIO 14  (HIGH = open; use relay module for barrier)
 *
 ...
 *
 * Laravel API:
 *   POST {API_BASE}/api/rfid/scan
 *   Header: X-RFID-TOKEN: <same as RFID_API_TOKEN in .env>
 *   Body: { "uid": "AABBCCDD", "gate_id": "GATE-IN-1", "direction": "Entry" }
 *
 * Responses used:
 *   Access Granted  → green LED + open gate
 *   anything else   → red LED + buzzer
 */
...
// Unique per reader: Entry gate vs Exit gate
const char* GATE_ID   = "GATE-IN-1";
const char* DIRECTION = "Entry";   // "Entry" or "Exit"
// =====================================
...
String postScan(const String &uid) {
  ...
  // Prefer boolean granted — status string alone can mismatch on older payloads
